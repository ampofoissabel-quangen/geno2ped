...
build_pedigree <- function(
  ...
  if (use_age) {
    ...
    off_by    <- by[candidates$Offspring]
    par_by    <- by[candidates$Parent]

    keep <- !is.na(off_by) & !is.na(par_by) &
      par_by <= off_by - min_parent_gap
  ...
  for (o in ids) {
    ...
    if (!is.data.frame(s_c) || !is.data.frame(d_c)) next
  ...
  if (nrow(best_trios)) {
    ...
    best_trios$decision_reason <- ifelse(
      best_trios$decision == "accept",
      "passed",
      "high_trio_mendelian_error"
    )
  } else {
  ...
  ped$assignment_status      <- ifelse(
    ped$Sire != 0 & ped$Dam != 0, "complete", "unassigned"
  )
