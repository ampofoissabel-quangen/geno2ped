...
## Overview
`geno2ped` is an R package for rapid pedigree construction from SNP genotype data.
It supports PLINK (bed/bim/fam) and VCF/VCF.GZ input formats and infers
familial relationships from genomic similarity.
...
## Output
`read_genotypes()` returns a list with:
- `$G` — numeric matrix of genotypes (0/1/2), rows = samples, cols = SNPs
- `$ids` — character vector of sample IDs
- `$map` — data.frame with columns `chr`, `pos`, `snp`
- `$meta` — data.frame of sample metadata (or `NULL` if not provided)
